    ...
    if "password_correct" not in st.session_state:
        ...
        <div style='text-align: center; padding: 50px;'>
            <div style='background: linear-gradient(135deg, #1e1b4b 0%, #4c1d95 100%); 
                ...
                <h1 style='color: #a78bfa; margin-bottom: 10px;'>NYZTrade Pro</h1>
                <p style='color: #e2e8f0; font-size: 18px;'>Professional Stock Valuation & Screening Platform</p>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def fetch_stock_data(ticker):
    try:
        time.sleep(0.5)  # Reduced delay for screener
...
def run_screener(criteria):
    """
    Run stock screener based on criteria
    
    criteria = {
        'cap_type': ['Large Cap', 'Mid Cap', 'Small Cap'],  # Optional
        'valuation': 'undervalued' or 'overvalued',  # Optional
        'upside_min': float,  # Minimum upside %
        'upside_max': float,  # Maximum upside %
        'price_min': float,  # Minimum price
        'price_max': float,  # Maximum price
        'pe_max': float,  # Maximum PE ratio
        'near_52w_high': True/False,  # Within 10% of 52W high
        'near_52w_low': True/False,  # Within 10% of 52W low
    ...
    for idx, (ticker, name) in enumerate(all_stocks.items()):
        # Update progress
        progress = (idx + 1) / total
        progress_bar.progress(progress)
        status_text.text(f"Screening {idx + 1}/{total}: {ticker}")
        ...
        if passes:
            ...
            # Check limit
            if 'limit' in criteria and len(results) >= criteria['limit']:
                break
...
# ============================================================================
PRESET_SCREENERS = {
    "🚀 Highly Undervalued Large Caps": {
        'cap_type': ['Large Cap'],
        'valuation': 'undervalued',
        'upside_min': 25,
        'limit': 50
    },
    "📈 Highly Undervalued Mid Caps": {
        'cap_type': ['Mid Cap'],
        'valuation': 'undervalued',
        'upside_min': 30,
        'limit': 50
    },
    "💎 Highly Undervalued Small Caps": {
        'cap_type': ['Small Cap'],
        'valuation': 'undervalued',
        'upside_min': 35,
        'limit': 50
    ...
    "⚠️ Overvalued Mid Caps": {
        'cap_type': ['Mid Cap'],
        'valuation': 'overvalued',
        'upside_max': -20,
        'limit': 50
    },
    "⚠️ Overvalued Small Caps": {
        'cap_type': ['Small Cap'],
        'valuation': 'overvalued',
        'upside_max': -25,
        'limit': 50
...
<div class="company-header" style="text-align: center;">
    <div class="company-name">NYZTRADE STOCK VALUATION & SCREENER PRO</div>
    <div class="metric-label">📊 7500+ Stocks | Advanced Screening | Multi-Factor Analysis | Real-Time Data</div>
</div>
''', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    mode = st.radio(
        "📊 Mode",
        ["🔍 Stock Screener", "📈 Individual Analysis"],
        help="Choose between screening multiple stocks or analyzing one stock in detail"
    )
    
    if mode == "🔍 Stock Screener":
        st.markdown("### 🎯 Screener Type")
        
        screener_type = st.radio(
            "Select Screener",
            ["Preset Screeners", "Custom Filters"]
        )
        ...
        if screener_type == "Preset Screeners":
            ...
            # Upside Range
            st.markdown("**Expected Upside %**")
            upside_min = st.number_input("Minimum Upside %", value=0.0, step=5.0)
            upside_max = st.number_input("Maximum Upside %", value=100.0, step=5.0)
            
            # Price Range
            st.markdown("**Price Range (₹)**")
            price_min = st.number_input("Min Price", value=0.0, step=10.0)
            price_max = st.number_input("Max Price", value=10000.0, step=100.0)
            
            # PE Filter
            pe_max = st.number_input("Maximum PE Ratio", value=50.0, step=5.0)
            
            # 52-Week Position
            near_52w_high = st.checkbox("Near 52-Week High (within 10%)")
            near_52w_low = st.checkbox("Near 52-Week Low")
            
            # Result Limit
            result_limit = st.number_input("Max Results", value=50, min_value=10, max_value=200, step=10)
        ...
        <div class="metric-card" style="text-align: center;">
            <div class="metric-label">📊 {len(all_stocks):,} Stocks Available</div>
        </div>
        ''', unsafe_allow_html=True)
...
if mode == "🔍 Stock Screener":
    if 'run_screen' in locals() and run_screen:
        ...
        if screener_type == "Preset Screeners":
            ...
            st.markdown(f'<div class="section-header">🔍 Screening: {selected_preset}</div>', unsafe_allow_html=True)
        else:
            criteria = {}
            if cap_types:
            ...
            criteria['limit'] = result_limit
            
            st.markdown('<div class="section-header">🔍 Custom Screener Results</div>', unsafe_allow_html=True)
        
        # Run screener
        results_df = run_screener(criteria)
        ...
        if results_df.empty:
            ...
            display_df['Market Cap'] = display_df['Market Cap'].apply(lambda x: f"₹{x:,.0f}Cr")
            ...
            display_df['From 52W Low %'] = display_df['From 52W Low %'].apply(lambda x: f"{x:+.2f}%" if pd.notna(x) else 'N/A')
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "From 52W Low %": st.column_config.TextColumn("vs Low", width="small"),
            ...
            st.markdown("---")
            st.markdown("### 📊 Analyze a Stock from Results")
            ...
            if st.button("📈 Analyze Selected Stock", use_container_width=True, type="primary"):
                st.session_state.analyze = selected_ticker

else:  # Individual Analysis Mode
    if 'analyze_clicked' in locals() and analyze_clicked:
        st.session_state.analyze = custom.upper() if custom else ticker

# Individual Stock Analysis (shared between both modes)
if 'analyze' in st.session_state and st.session_state.analyze:
    ...
    if error or not info:
        ...
        <div class="metric-card">
            <div class="metric-label"><b>Troubleshooting Tips:</b></div>
            <ul>
                <li>Verify the ticker symbol is correct (e.g., RELIANCE.NS for NSE)</li>
                <li>Check your internet connection</li>
                <li>Try again in a few moments (API rate limits may apply)</li>
            </ul>
        </div>
        ''', unsafe_allow_html=True)
        st.stop()
    ...
    with chart_col3:
        st.markdown('<div class="section-header">📍 52-Week Position</div>', unsafe_allow_html=True)
        if vals['pct_from_high'] is not None:
            st.markdown(f'''
            <div class="metric-card">
                <div class="metric-label">Distance from 52W High</div>
                <div class="metric-value">{-vals['pct_from_high']:+.2f}%</div>
                <div class="metric-label" style="margin-top: 15px;">Distance from 52W Low</div>
                <div class="metric-value">{vals['pct_from_low']:+.2f}%</div>
                <div class="metric-label" style="margin-top: 15px;">
                    52W Range: ₹{vals['52w_low']:,.2f} - ₹{vals['52w_high']:,.2f}
                </div>
            </div>
            ''', unsafe_allow_html=True)
        else:
    ...
    with val_col1:
        if vals['fair_value_pe'] and vals['trailing_pe']:
            ...
            st.markdown(f'''
            <div class="metric-card">
                <div class="metric-label" style="font-size: 18px; margin-bottom: 15px;">📈 PE Multiple Method</div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span>Current PE</span>
                    <span style="color: #a78bfa;">{vals['trailing_pe']:.2f}x</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span>Industry PE</span>
                    <span style="color: #a78bfa;">{vals['industry_pe']:.2f}x</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span>EPS (TTM)</span>
                    <span style="color: #a78bfa;">₹{vals['trailing_eps']:.2f}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span>Fair Value (PE)</span>
                    <span style="color: {upside_color};">₹{vals['fair_value_pe']:,.2f}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span><b>Upside (PE)</b></span>
                    <span style="color: {upside_color}; font-weight: bold;">{vals['upside_pe']:+.2f}%</span>
                </div>
            </div>
            ''', unsafe_allow_html=True)
        else:
    ...
    with val_col2:
        if vals['fair_value_ev'] and vals['current_ev_ebitda']:
            ...
            st.markdown(f'''
            <div class="metric-card">
                <div class="metric-label" style="font-size: 18px; margin-bottom: 15px;">💼 EV/EBITDA Method</div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span>Current EV/EBITDA</span>
                    <span style="color: #a78bfa;">{vals['current_ev_ebitda']:.2f}x</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span>Industry EV/EBITDA</span>
                    <span style="color: #a78bfa;">{vals['industry_ev_ebitda']:.2f}x</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span>EBITDA</span>
                    <span style="color: #a78bfa;">₹{vals['ebitda']/10000000:,.0f} Cr</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span>Fair Value (EV)</span>
                    <span style="color: {upside_color_ev};">₹{vals['fair_value_ev']:,.2f}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin: 10px 0;">
                    <span><b>Upside (EV)</b></span>
                    <span style="color: {upside_color_ev}; font-weight: bold;">{vals['upside_ev']:+.2f}%</span>
                </div>
            </div>
            ''', unsafe_allow_html=True)
        else:
    ...
    financial_data = pd.DataFrame({
        'Metric': [
            'Current Price', 'Market Cap', 'Cap Type', 'Enterprise Value', 
            'PE Ratio (TTM)', 'Forward PE', 'EV/EBITDA',
            'P/B Ratio', 'P/S Ratio', 'EPS (TTM)',
            'EBITDA', 'Book Value', 'Net Debt',
            '52W High', '52W Low', 'Distance from 52W High',
            'Distance from 52W Low', 'Beta', 'Dividend Yield',
            'ROE', 'Profit Margin'
        ],
        'Value': [
            f"₹{vals['price']:,.2f}",
            ...
            f"{-vals['pct_from_high']:+.2f}%" if vals['pct_from_high'] else 'N/A',
            f"{vals['pct_from_low']:+.2f}%" if vals['pct_from_low'] else 'N/A',
...
<div style="text-align: center; margin-top: 50px; padding: 30px; 
    ...
    <div style="font-size: 18px; font-weight: 700; color: #a78bfa; margin-bottom: 10px;">
        NYZTrade Pro | Professional Stock Valuation & Screening Platform
    </div>
